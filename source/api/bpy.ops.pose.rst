...
.. function:: breakdown(percentage=0.5, prev_frame=0, next_frame=0, channels='ALL', axis_lock='FREE')

   Create a suitable breakdown pose on the current frame

   :arg percentage:

      Percentage, Weighting factor for which keyframe is favored more

   :type percentage: float in [0, 1], (optional)
   :arg prev_frame:

      Previous Keyframe, Frame number of keyframe immediately before the current frame

   :type prev_frame: int in [-1048574, 1048574], (optional)
   :arg next_frame:

      Next Keyframe, Frame number of keyframe immediately after the current frame

   :type next_frame: int in [-1048574, 1048574], (optional)
   :arg channels:
   ...
   :type channels: enum in ['ALL', 'LOC', 'ROT', 'SIZE', 'BBONE', 'CUSTOM'], (optional)
   :arg axis_lock:
   ...
   :type axis_lock: enum in ['FREE', 'X', 'Y', 'Z'], (optional)
...
.. function:: group_deselect()

   Deselect bones of active Bone Group

.. function:: group_move(direction='UP')

   Change position of active Bone Group in list of Bone Groups

   :arg direction:

      Direction, Direction to move the active Bone Group towards
...
.. function:: ik_add(with_targets=True)
   ...
   :arg with_targets:
      ...
      With Targets, Assign IK Constraint with targets derived from the select bones/objects
...
.. function:: paste(flipped=False, selected_mask=False)
   ...
   :arg selected_mask:
      ...
      On Selected Only, Only paste the stored pose on to selected bones in the current pose

   :type selected_mask: boolean, (optional)

.. function:: paths_calculate(start_frame=1, end_frame=250, bake_location='TAILS')

   Calculate paths for the selected bones

   :arg start_frame:

      Start, First frame to calculate bone paths on

   :type start_frame: int in [-1048574, 1048574], (optional)
   :arg end_frame:

      End, Last frame to calculate bone paths on
...
.. function:: paths_clear(only_selected=False)
   ...
   :arg only_selected:
      ...
      Only Selected, Only clear paths from selected bones
...
.. function:: propagate(mode='WHILE_HELD', end_frame=250.0)
   ...
   :arg end_frame:
      ...
      End Frame, Frame to stop propagating frames to (for 'Before Frame' mode)

   :type end_frame: float in [1.17549e-38, inf], (optional)

.. function:: push(percentage=0.5, prev_frame=0, next_frame=0, channels='ALL', axis_lock='FREE')
...
.. function:: relax(percentage=0.5, prev_frame=0, next_frame=0, channels='ALL', axis_lock='FREE')
...
.. function:: select_grouped(extend=False, type='LAYER')
...
.. function:: user_transforms_clear(only_selected=True)
   ...
   :arg only_selected:
      ...
      Only Selected, Only visible/selected bones
